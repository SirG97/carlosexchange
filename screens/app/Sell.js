...
const Sell= ({ navigation }) => {
    ...
    const[configSellRate, setConfigSellRate] = React.useState(0)
      ...
      try {
        ...
        await axios.get(url, header).then((response) => {
          ...
          if(status !== 'success'){
            ...
            setConfigSellRate(data.config.sell_rate) 
    ...
    const validateInput = (selectedCoin, selectedCoinNetwork, amount) =>{
        ...
        if(selectedCoin == '' || selectedCoinNetwork == '' || amount == false){
            setInvalid(true)
        }else{
            setInvalid(false)
        }
    ...
    const startTransaction = async() => {
      setLoading(true)
       const url = 'https://app.carlosexchange.com/api/user/sell/order';
       const header = {
        headers:{
          Authorization: `Bearer  ${storedCredentials.token}`,
          'auth-token': storedCredentials.token
       ...
       const details = {
            ...
            currency: currency,
            amount: amount,
            bank: storedCredentials.user.bank_code,
            bank_name: storedCredentials.user.bank,
            account_number: storedCredentials.user.account_number,
            account_name: storedCredentials.user.account_name
       }

       console.log('Details sent are ', details, 'Bank is supposed to show ', storedCredentials.user.bank);
       ...
       //    console.log(`The message is ${message}, \nStatus is  ${status}, \n User ID is ${data.user_id}`);
           if(status !== 'success'){
            ...
            //   persistLogin({...data}, message, status)
             navigation.navigate('PendingSellTransaction', {data: data})
           }
           setLoading(false)
        ...
        <ScrollView style={styles.container}>
          <View style={styles.header}>
            <View style={styles.pageTitleContainer}>
              <Text style={styles.pageTitle}>Sell</Text>
            </View>
            ...
            >
              <TextInput
                ...
                onChangeText={(selectedCoin) => {
                  ...
                  validateInput(selectedCoin, selectedCoinNetwork, amount);
                }}
